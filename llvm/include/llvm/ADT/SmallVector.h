...
public:
  template <class Si>
  __attribute__((always_inline))
  static void noalias_helper_memcpy(const T* EltPtr, T* __restrict Dst, Si* __restrict Size) {
    memcpy(reinterpret_cast<void *>(Dst), EltPtr, sizeof(T));
    *Size += 1;
  }
  ...
  void push_back(ValueParamT Elt) {
    const T *EltPtr = reserveForParamAndGetAddress(Elt);
    noalias_helper_memcpy(EltPtr, this->end(), &this->Size);
